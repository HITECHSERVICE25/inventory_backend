...
class TechnicianService {
  ...
  async listTechnicians(filterParams = {}) {
    try {
      const { page = 1, limit = 10, search, ...filters } = filterParams;
      ...
      const query = Technician.find(filter)
        .skip(skip)
        .limit(Number(limit))
        .populate('companies', 'name');
